...
export default function Footer() {
  ...
  return (
    <footer className="bg-[#001529] text-white overflow-hidden">
      ...
      <div className="h-1 w-full bg-gradient-to-r from-[#0A2540] via-[#D4AF37] to-[#0A2540] opacity-80"></div>
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-12 gap-12 lg:gap-8">
          ...
          <div className="lg:col-span-4 space-y-6">
            ...
            <h3 className="text-2xl font-black tracking-tight text-white italic">
              Cerrajeros <span className="text-[#0A2540]">Villaviciosa 24h</span>
            </h3>
            ...
            <div className="flex gap-4 pt-2">
              <a href={`tel:${APP_PHONE}`} className="h-10 w-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#0A2540] transition-all group" aria-label="Llamar por teléfono a Cerrajeros Villaviciosa 24h">
                <Phone className="h-5 w-5 text-gray-400 group-hover:text-white" />
              </a>
              <a href={`mailto:${APP_EMAIL}`} className="h-10 w-10 rounded-full bg-white/5 border border-white/10 flex items-center justify-center hover:bg-[#0A2540] transition-all group" aria-label="Enviar correo electrónico">
          ...
          <div className="lg:col-span-2">
            <h4 className="text-sm font-bold uppercase tracking-[0.2em] text-[#0A2540] mb-6">Servicios</h4>
              ...
              {serviceLinks.map((link) => (
                <li key={link.href}>
                  <Link to={link.href} className="text-gray-400 hover:text-white transition-colors text-sm font-medium flex items-center gap-2 group">
                    <span className="h-0.5 w-0 bg-[#0A2540] transition-all group-hover:w-3"></span>
                    {link.label}
                  </Link>
                </li>
              ))}
          ...
          <div className="lg:col-span-3">
            <h4 className="text-sm font-bold uppercase tracking-[0.2em] text-[#0A2540] mb-6">Cobertura en Villaviciosa del Odón</h4>
            <div className="grid grid-cols-2 gap-y-3 gap-x-4">
              {zones.map((zone) => (
                <Link
                  key={zone}
                  href={`/cerrajeros-${slugify(zone)}`}
                  className="text-[13px] text-gray-300 hover:text-[#D4AF37] transition-colors whitespace-nowrap"
          ...
          <div className="lg:col-span-3 space-y-6">
            <h4 className="text-sm font-bold uppercase tracking-[0.2em] text-[#0A2540] mb-6">Atención al Cliente</h4>
            <div className="space-y-4">
              <div className="bg-white/5 p-4 rounded-2xl border border-white/10 hover:border-[#0A2540]/30 transition-all group">
              ...
              <div className="bg-gradient-to-br from-[#0A2540] to-[#D4AF37] p-4 rounded-2xl shadow-lg shadow-amber-500/10">
